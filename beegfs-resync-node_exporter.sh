...
output="${textfile_dir}/beegfs_resync_${nodetype}_${mirrorid}"

timeout=59

if [ "$#" -ne 4 ]
then
  echo "Usage: $0 <nodetype> <groupid> <clustername> <textfiledir>"
  exit 1
fi
...
function _get_resync_metrics
{
  ...
  timeout ${timeout} ${beegfs_ctl} ${beegfs_cmd} --nodetype=${nodetype} --mirrorgroupid=${mirrorid}
}
